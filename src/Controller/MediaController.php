...
final class MediaController extends AbstractController
{
    ...
    #[Route('/api/media/{id}', name: 'api_get_media', methods: ['GET'])]
    public function get(Media                  $id,
                        UrlGeneratorInterface  $urlGenerator,
                        EntityManagerInterface $entityManager,
                        SerializerInterface    $serializer): JsonResponse
    {
        $location = $urlGenerator->generate('app_media', [], UrlGeneratorInterface::ABSOLUTE_URL);
        $location = $location . str_replace('/public/', "", $id->getPublicPath() . "/" . $id->getRealPath());

        return $id ?
            new JsonResponse($serializer->serialize($id, "json", []),
        ...
        UrlGeneratorInterface  $urlGenerator): JsonResponse
    {
        ...
        $location = $urlGenerator->generate('app_get_media',
            ["media" => $media->getId()],
